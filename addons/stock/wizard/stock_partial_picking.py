...
# Compiled at: 2018-12-03 14:40:11
...
class stock_partial_picking_line(osv.TransientModel):
    ...
    _columns = {'product_id': fields.many2one('product.product', 4='Product', 6=True, 7='CASCADE'), 'quantity': fields.float('Quantity', 10=dp.get_precision('Product UoS'), 6=True), 
...
class stock_partial_picking(osv.osv_memory):
    ...
    _columns = {'date': fields.datetime('Date', 5=True), 'user_id': fields.many2one('res.users', 'Người lập phiếu', 5=True), 
    ...
    def _partial_move_for(self, cr, uid, move):
        ...
        if move.state != 'cancel' and move.check:
            partial_move = {'product_id': move.product_id.id, 'quantity': move.product_qty if move.state == 'assigned' else 0, 'product_uom': move.product_uom.id, 
    ...
    def do_partial(self, cr, uid, ids, context=None):
        ...
        for wizard_line in partial.move_ids:
            ...
            if type == 'out':
                ...
                if not kehoach or kehoach is None:
                    ...
                    if sale_order_lineobj:
                        ...
                        tru = round(sl_giao, 4) - round(ton_lenh, 4)
                        if ton_lenh < 0 or round(sl_giao, 4) > round(ton_lenh, 4) and ton_lenh > 0 and tru != 0:
                    ...
                    if chitiet_khobj:
                        ...
                        sl_giao = wizard_line.quantity
                        if ton_lenh < 0 or round(sl_giao, 4) > round(ton_lenh, 4) and ton_lenh >= 0:
            ...
            partial_data['move%s' % move_id] = {'product_id': wizard_line.product_id.id, 'product_qty': wizard_line.quantity, 'product_uom': wizard_line.product_uom.id, 
               'prodlot_id': wizard_line.prodlot_id.id}
        ...
        if partial.picking_id.type == 'out':
            if delivered_picking:
                return {'name': _('Phiếu xuất kho đã hoàn tất'), 'domain': "[('id', 'in', [" + str(delivered_picking) + "]),('type','=','out'),('state','=','done'),('mistake_delivery','=',False)]", 
                   'view_type': 'form', 
                   'view_mode': 'tree,form', 
            ...
            return {'name': _('Phiếu nhập kho đã hoàn tất'), 'domain': "[('name', '=', ['" + partial.picking_id.name + "'])]", 
               'view_type': 'form', 
               'view_mode': 'tree,form', 
            ...
            return {'name': _('Phiếu luân chuyển kho đã hoàn tất'), 'domain': "[('name', '=', ['" + partial.picking_id.name + "'])]", 
               'view_type': 'form', 
               'view_mode': 'tree,form', 
