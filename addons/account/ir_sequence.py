...
class ir_sequence_fiscalyear(osv.osv):
    ...
    _columns = {'sequence_id': fields.many2one('ir.sequence', 'Sequence', required=True, ondelete='cascade'),
    ...
    _sql_constraints = [('main_id', 'CHECK (sequence_main_id != sequence_id)', 'Main Sequence must be different from current !')]
...
class ir_sequence(osv.osv):
    ...
    _columns = {'fiscal_ids': fields.one2many('account.sequence.fiscalyear', 'sequence_main_id', 'Sequences')}
