...
class account_automatic_reconcile(osv.osv_memory):
    ...
    _columns = {'account_ids': fields.many2many('account.account', 'reconcile_account_rel', 'reconcile_id', 'account_id', 'Accounts to Reconcile', domain=[('reconcile', '=', 1)]),
    ...
    _defaults = {'reconciled': _get_reconciled,
        ...
        def check2(value, move_list, power):
            ...
            def check(value, move_list, power):
                for i in range(len(move_list)):
                    move = move_list[i]
                    if power == 1:
                        if abs(value - move[1]) <= max_amount + 1e-05:
                            return [move[0]]
                    else:
                        del move_list[i]
    ...
    def reconcile(self, cr, uid, ids, context = None):
        ...
        for account_id in form.account_ids:
            ...
            for partner_id in partner_ids:
                cr.execute("SELECT id FROM account_move_line WHERE account_id=%s AND partner_id=%s AND state <> 'draft' AND reconcile_id IS NULL", (account_id.id, partner_id))
            ...
            cr.execute("SELECT partner_id FROM account_move_line WHERE account_id=%s AND reconcile_id IS NULL AND state <> 'draft' AND partner_id IS NOT NULL GROUP BY partner_id HAVING count(*)>1", (account_id.id,))
            partner_ids = [ id for id, in cr.fetchall() ]
            for partner_id in partner_ids:
                cr.execute("SELECT id, debit FROM account_move_line WHERE account_id=%s AND partner_id=%s AND reconcile_id IS NULL AND state <> 'draft' AND debit > 0 ORDER BY date_maturity", (account_id.id, partner_id))
                debits = cr.fetchall()
                cr.execute("SELECT id, credit FROM account_move_line WHERE account_id=%s AND partner_id=%s AND reconcile_id IS NULL AND state <> 'draft' AND credit > 0 ORDER BY date_maturity", (account_id.id, partner_id))
                credits = cr.fetchall()
            ...
            cr.execute("SELECT count(*) FROM account_move_line WHERE account_id=%s AND reconcile_id IS NULL AND state <> 'draft' " + partner_filter, (account_id.id,))
