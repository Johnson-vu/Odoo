...
class account_move_line_reconcile(osv.osv_memory):
    ...
    _columns = {'trans_nbr': fields.integer('# of Transaction', readonly=True),
    ...
    def trans_rec_reconcile_full(self, cr, uid, ids, context = None):
        ...
        if ids:
            period_id = ids[0]
        account_move_line_obj.reconcile(cr, uid, context['active_ids'], 'manual', account_id, period_id, journal_id, context=context)
...
class account_move_line_reconcile_writeoff(osv.osv_memory):
    ...
    _columns = {'journal_id': fields.many2one('account.journal', 'Write-Off Journal', required=True),
    ...
    _defaults = {'date_p': lambda *a: time.strftime('%Y-%m-%d'),
     'comment': 'Write-off'}
    ...
    def trans_rec_addendum(self, cr, uid, ids, context = None):
        ...
        return {'name': _('Reconcile Writeoff'),
         'context': context,
         'view_type': 'form',
         'view_mode': 'form',
