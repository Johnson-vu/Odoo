...
class account_partner_reconcile_process(osv.osv_memory):
    ...
    def _get_today_reconciled(self, cr, uid, context = None):
        cr.execute("SELECT l.partner_id FROM account_move_line AS l LEFT JOIN res_partner p ON (p.id = l.partner_id) WHERE l.reconcile_id IS NULL AND %s =  to_char(p.last_reconciliation_date, 'YYYY-MM-DD') AND l.state <> 'draft' GROUP BY l.partner_id ", (time.strftime('%Y-%m-%d'),))
        return len(map(lambda x: x[0], cr.fetchall())) + 1
    ...
    _columns = {'to_reconcile': fields.float('Remaining Partners', readonly=True, help='This is the remaining partners for who you should check if there is something to reconcile or not. This figure already count the current partner as reconciled.'),
     ...
     'next_partner_id': fields.many2one('res.partner', 'Next Partner to Reconcile', readonly=True, help='This field shows you the next partner that will be automatically chosen by the system to go through the reconciliation process, based on the latest day it have been reconciled.')}
    _defaults = {'to_reconcile': _get_to_reconcile,
