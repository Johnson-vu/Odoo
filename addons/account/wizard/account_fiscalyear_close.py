...
class account_fiscalyear_close(osv.osv_memory):
    ...
    _defaults = {'report_name': lambda self, cr, uid, context: _('End of Fiscal Year Entry')}
    ...
    def data_save(self, cr, uid, ids, context = None):
        ...
        def _reconcile_fy_closing(cr, uid, ids, context = None):
            """
            This private function manually do the reconciliation on the account_move_line given as `ids\xc2\xb4, and directly
            through psql. It's necessary to do it this way because the usual `reconcile()\xc2\xb4 function on account.move.line
        ...
        if not new_journal.centralisation or new_journal.entry_posted:
            raise osv.except_osv(_('User Error!'), _('The journal must have centralized counterpart without the Skipping draft state option checked.'))
        move_ids = obj_acc_move.search(cr, uid, [('journal_id', '=', new_journal.id), ('period_id', '=', period.id)])
        if move_ids:
        ...
        query_line = obj_acc_move_line._query_get(cr, uid, obj='account_move_line', context={'fiscalyear': fy_ids})
        vals = {'name': '/',
         'ref': '',
         'period_id': period.id,
        ...
        for account in obj_acc_account.browse(cr, uid, account_ids, context={'fiscalyear': fy_id}):
            ...
            if account.currency_id:
                cr.execute('SELECT sum(COALESCE(amount_currency,0.0)) as balance_in_currency FROM account_move_line WHERE account_id = %s AND ' + query_line + ' AND currency_id = %s', (account.id, account.currency_id.id))
        ...
        ids = obj_acc_move_line.search(cr, uid, [('journal_id', '=', new_journal.id), ('period_id.fiscalyear_id', '=', new_fyear.id)])
        if ids:
        ...
        if not ids:
            ids = [obj_acc_journal_period.create(cr, uid, {'name': (new_journal.name or '') + ':' + (period.code or ''),
              'journal_id': new_journal.id,
              'period_id': period.id})]
        cr.execute('UPDATE account_fiscalyear SET end_journal_period_id = %s WHERE id = %s', (ids[0], old_fyear.id))
