...
class account_invoice_refund(osv.osv_memory):
    ...
    _columns = {'date': fields.date('Date', help='This date will be used as the invoice date for credit note and period will be chosen accordingly!'),
    ...
    def _get_journal(self, cr, uid, context = None):
        ...
        type = inv_type == 'out_invoice' and 'sale_refund' or inv_type == 'out_refund' and 'sale' or inv_type == 'in_invoice' and 'purchase_refund' or inv_type == 'in_refund' and 'purchase'
    ...
    def fields_view_get(self, cr, uid, view_id = None, view_type = False, context = None, toolbar = False, submenu = False):
        ...
        company_id = user_obj.browse(cr, uid, uid, context=context).company_id.id
        journal_type = type == 'out_invoice' and 'sale_refund' or type == 'out_refund' and 'sale' or type == 'in_invoice' and 'purchase_refund' or type == 'in_refund' and 'purchase'
    ...
    def compute_refund(self, cr, uid, ids, mode = 'refund', context = None):
        ...
        @param ids: the account invoice refund\xe2\x80\x99s ID or list of IDs
        ...
        for form in self.browse(cr, uid, ids, context=context):
            ...
            for inv in inv_obj.browse(cr, uid, context.get('active_ids'), context=context):
                ...
                if form.date:
                    ...
                    if not form.period.id:
                        ...
                        if result_query:
                            cr.execute('select p.id from account_fiscalyear y, account_period p where y.id=p.fiscalyear_id                                     and date(%s) between p.date_start AND p.date_stop and y.company_id = %s limit 1', (date, company.id))
                        else:
                            cr.execute('SELECT id\n                                        from account_period where date(%s)\n                                        between date_start AND  date_stop                                          limit 1 ', (date,))
                        res = cr.fetchone()
                        if res:
                            period = res[0]
                else:
                ...
                if mode in ('cancel', 'modify'):
                    ...
                    for account in to_reconcile_ids:
                        account_m_line_obj.reconcile(cr, uid, to_reconcile_ids[account], writeoff_period_id=period, writeoff_journal_id=inv.journal_id.id, writeoff_acc_id=inv.account_id.id)
            ...
            xml_id = inv.type == 'out_refund' and 'action_invoice_tree1' or inv.type == 'in_refund' and 'action_invoice_tree2' or inv.type == 'out_invoice' and 'action_invoice_tree3' or inv.type == 'in_invoice' and 'action_invoice_tree4'
