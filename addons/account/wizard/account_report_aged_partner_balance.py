...
class account_aged_trial_balance(osv.osv_memory):
    ...
    _columns = {'period_length': fields.integer('Period Length (days)', required=True),
     'direction_selection': fields.selection([('past', 'Past'), ('future', 'Future')], 'Analysis Direction', required=True),
    ...
    def _print_report(self, cr, uid, ids, data, context = None):
        ...
        if data['form']['direction_selection'] == 'past':
            for i in range(5)[::-1]:
                ...
                res[str(i)] = {'name': i != 0 and str((5 - (i + 1)) * period_length) + '-' + str((5 - i) * period_length) or '+' + str(4 * period_length),
            ...
            for i in range(5):
                stop = start + relativedelta(days=period_length)
                res[str(5 - (i + 1))] = {'name': i != 4 and str(i * period_length) + '-' + str((i + 1) * period_length) or '+' + str(4 * period_length),
