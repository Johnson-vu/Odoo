...
class account_move_line_reconcile_select(osv.osv_memory):
    ...
    _columns = {'account_id': fields.many2one('account.account', 'Account', domain=[('reconcile', '=', 1)], required=True)}
    ...
    def action_open_window(self, cr, uid, ids, context = None):
        ...
        @param ids: account move line reconcile select\xe2\x80\x99s ID or list of IDs
        @return: dictionary of  Open  account move line window for reconcile on given account id
        
         """
        data = self.read(cr, uid, ids, context=context)[0]
        return {'domain': "[('account_id','=',%d),('reconcile_id','=',False),('state','<>','draft')]" % data['account_id'],
