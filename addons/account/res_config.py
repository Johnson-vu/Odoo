...
class account_config_settings(osv.osv_memory):
    ...
    _columns = {'company_id': fields.many2one('res.company', 'Company', required=True),
     ...
     'expects_chart_of_accounts': fields.related('company_id', 'expects_chart_of_accounts', type='boolean', string='This company has its own chart of accounts', help='Check this box if this company is a legal entity.'),
     'currency_id': fields.related('company_id', 'currency_id', type='many2one', relation='res.currency', required=True, string='Default company currency', help='Main currency of the company.'),
     'paypal_account': fields.related('company_id', 'paypal_account', type='char', size=128, string='Paypal account', help='Paypal account (email) for receiving online payments (credit card, etc.) If you set a paypal account, the customer  will be able to pay your invoices or quotations with a button "Pay with  Paypal" in automated emails or through the OpenERP portal.'),
     'company_footer': fields.related('company_id', 'rml_footer', type='text', readonly=True, string='Bank accounts footer preview', help='Bank accounts as printed in the footer of each printed document'),
     ...
     'module_account_check_writing': fields.boolean('Pay your suppliers by check', help='This allows you to check writing and printing.\n                This installs the module account_check_writing.'),
     'module_account_accountant': fields.boolean('Full accounting features: journals, legal statements, chart of accounts, etc.', help='If you do not check this box, you will be able to do invoicing & payments, but not accounting (Journal Items, Chart of  Accounts, ...)'),
     'module_account_asset': fields.boolean('Assets management', help='This allows you to manage the assets owned by a company or a person.\n                It keeps track of the depreciation occurred on those assets, and creates account move for those depreciation lines.\n                This installs the module account_asset. If you do not check this box, you will be able to do invoicing & payments,\n                but not accounting (Journal Items, Chart of Accounts, ...)'),
     'module_account_budget': fields.boolean('Budget management', help='This allows accountants to manage analytic and crossovered budgets.\n                Once the master budgets and the budgets are defined,\n                the project managers can set the planned amount on each analytic account.\n                This installs the module account_budget.'),
     'module_account_payment': fields.boolean('Manage payment orders', help='This allows you to create and manage your payment orders, with purposes to\n                    * serve as base for an easy plug-in of various automated payment mechanisms, and\n                    * provide a more efficient way to manage invoice payments.\n                This installs the module account_payment.'),
     'module_account_voucher': fields.boolean('Manage customer payments', help='This includes all the basic requirements of voucher entries for bank, cash, sales, purchase, expense, contra, etc.\n                This installs the module account_voucher.'),
     'module_account_followup': fields.boolean('Manage customer payment follow-ups', help='This allows to automate letters for unpaid invoices, with multi-level recalls.\n                This installs the module account_followup.'),
     'group_proforma_invoices': fields.boolean('Allow pro-forma invoices', implied_group='account.group_proforma_invoices', help='Allows you to put invoices in pro-forma state.'),
     'default_sale_tax': fields.many2one('account.tax', 'Default sale tax', help='This sale tax will be assigned by default on new products.'),
     'default_purchase_tax': fields.many2one('account.tax', 'Default purchase tax', help='This purchase tax will be assigned by default on new products.'),
     'decimal_precision': fields.integer('Decimal precision on journal entries', help='As an example, a decimal precision of 2 will allow journal entries  like: 9.99 EUR, whereas a decimal precision of 4 will allow journal  entries like: 0.0231 EUR.'),
     'group_multi_currency': fields.boolean('Allow multi currencies', implied_group='base.group_multi_currency', help='Allows you multi currency environment'),
     'group_analytic_accounting': fields.boolean('Analytic accounting', implied_group='analytic.group_analytic_accounting', help='Allows you to use the analytic accounting.'),
     'group_check_supplier_invoice_total': fields.boolean('Check the total of supplier invoices', implied_group='account.group_supplier_inv_check_total')}
    ...
    def _get_default_fiscalyear_data(self, cr, uid, company_id, context = None):
        ...
        """
        fiscalyear_ids = self.pool.get('account.fiscalyear').search(cr, uid, [('date_start', '<=', time.strftime(DF)), ('date_stop', '>=', time.strftime(DF)), ('company_id', '=', company_id)])
        if fiscalyear_ids:
            fiscalyear = self.pool.get('account.fiscalyear').browse(cr, uid, fiscalyear_ids[0], context=context)
            if len(fiscalyear.period_ids) == 5:
                period = '3months'
            else:
                period = 'month'
            return (fiscalyear.date_start, fiscalyear.date_stop, period)
        else:
            past_fiscalyear_ids = self.pool.get('account.fiscalyear').search(cr, uid, [('date_stop', '<=', time.strftime(DF)), ('company_id', '=', company_id)])
    ...
    def onchange_company_id(self, cr, uid, ids, company_id, context = None):
        ...
        if company_id:
            ...
            fiscalyear_count = self.pool.get('account.fiscalyear').search_count(cr, uid, [('date_start', '<=', time.strftime('%Y-%m-%d')), ('date_stop', '>=', time.strftime('%Y-%m-%d')), ('company_id', '=', company_id)])
            date_start, date_stop, period = self._get_default_fiscalyear_data(cr, uid, company_id, context=context)
            values = {'expects_chart_of_accounts': company.expects_chart_of_accounts,
            ...
            for journal in journal_obj.browse(cr, uid, journal_ids):
                if journal.type in ('sale', 'sale_refund', 'purchase', 'purchase_refund'):
                    values.update({journal.type + '_journal_id': journal.id,
            ...
            values.update({'default_sale_tax': isinstance(taxes_id, list) and taxes_id[0] or taxes_id,
    ...
    def onchange_chart_template_id(self, cr, uid, ids, chart_template_id, context = None):
        ...
        res = {'value': {'complete_tax_set': False,
                   'sale_tax': False,
                   'purchase_tax': False,
                   'sale_tax_rate': 15,
        ...
        if chart_template_id:
            ...
            if chart_template.complete_tax_set:
                sale_tax_ids = tax_templ_obj.search(cr, uid, [('chart_template_id', '=', chart_template_id), ('type_tax_use', 'in', ('sale', 'all'))], order='sequence, id desc')
                purchase_tax_ids = tax_templ_obj.search(cr, uid, [('chart_template_id', '=', chart_template_id), ('type_tax_use', 'in', ('purchase', 'all'))], order='sequence, id desc')
    ...
    def set_default_taxes(self, cr, uid, ids, context = None):
        ...
        ir_values = self.pool.get('ir.values')
        config = self.browse(cr, uid, ids[0], context)
        ir_values.set_default(cr, uid, 'product.product', 'taxes_id', config.default_sale_tax and [5] or False, company_id=config.company_id.id)
        ir_values.set_default(cr, uid, 'product.product', 'supplier_taxes_id', config.default_purchase_tax and [2] or False, company_id=config.company_id.id)

    def set_chart_of_accounts(self, cr, uid, ids, context = None):
        """ install a chart of accounts for the given company (if required) """
        config = self.browse(cr, uid, ids[0], context)
        if not (config.chart_template_id and config.expects_chart_of_accounts and not config.has_chart_of_accounts):
            raise AssertionError
            wizard = self.pool.get('wizard.multi.charts.accounts')
            wizard_id = wizard.create(cr, uid, {'company_id': config.company_id.id,
             ...
             'currency_id': config.currency_id.id}, context)
    ...
    def set_fiscalyear(self, cr, uid, ids, context = None):
        ...
        if config.has_chart_of_accounts or config.chart_template_id:
            ...
            fiscalyear_count = fiscalyear.search_count(cr, uid, [('date_start', '<=', config.date_start), ('date_stop', '>=', config.date_stop), ('company_id', '=', config.company_id.id)], context=context)
