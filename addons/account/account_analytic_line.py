...
class account_analytic_line(osv.osv):
    ...
    _columns = {'product_uom_id': fields.many2one('product.uom', 'Unit of Measure'),
    ...
    _defaults = {'company_id': lambda self, cr, uid, c: self.pool.get('res.company')._company_default_get(cr, uid, 'account.analytic.line', context=c)}
    _order = 'date desc'
    ...
    def on_change_unit_amount(self, cr, uid, id, prod_id, quantity, company_id, unit = False, journal_id = False, context = None):
        ...
        if not journal_id or not prod_id:
            ...
            if j_id.type != 'sale':
                ...
                if not a:
                    raise osv.except_osv(_('Error!'), _('There is no expense account defined for this product: "%s" (id:%d).') % (prod.name, prod.id))
            else:
                a = prod.property_account_income.id
                if not a:
                    a = prod.categ_id.property_account_income_categ.id
                if not a:
                    raise osv.except_osv(_('Error!'), _('There is no income account defined for this product: "%s" (id:%d).') % (prod.name, prod_id))
            flag = False
...
class res_partner(osv.osv):
    ...
    _columns = {'contract_ids': fields.one2many('account.analytic.account', 'partner_id', 'Contracts', readonly=True)}
