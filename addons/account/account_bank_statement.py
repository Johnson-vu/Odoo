...
class account_bank_statement(osv.osv):
    ...
    def _currency(self, cursor, user, ids, name, args, context = None):
        ...
        currency_names = {}
        for currency_id, currency_name in res_currency_obj.name_get(cursor, user, [ x for x in res.values() ], context=context):
    ...
    _columns = {'name': fields.char('Reference', size=64, required=True, states={'draft': [('readonly', False)]}, readonly=True, help='if you give the Name other then /, its created Accounting Entries Move will be with same name as statement name. This allows the statement entries to have the same references than the statement itself'),
     ...
     'balance_start': fields.float('Starting Balance', digits_compute=dp.get_precision('Account'), states={'confirm': [('readonly', True)]}),
     'balance_end_real': fields.float('Ending Balance', digits_compute=dp.get_precision('Account'), states={'confirm': [('readonly', True)]}),
     'balance_end': fields.function(_end_balance, store={'account.bank.statement': (lambda self, cr, uid, ids, c = {}: ids, ['line_ids', 'move_line_ids', 'balance_start'], 10),
                     'account.bank.statement.line': (_get_statement, ['amount'], 10)}, string='Computed Balance', help='Balance as calculated based on Starting Balance and transaction lines'),
     ...
     'state': fields.selection([('draft', 'New'), ('open', 'Open'), ('confirm', 'Closed')], 'Status', required=True, readonly='1', help="When new statement is created the status will be 'Draft'.\nAnd after getting confirmation from the bank it will be in 'Confirmed' status."),
    ...
    def _prepare_move(self, cr, uid, st_line, st_line_number, context = None):
        ...
        """
        return {'journal_id': st_line.statement_id.journal_id.id,
    ...
    def _prepare_counterpart_move_line(self, cr, uid, st_line, move_id, amount, company_currency_id, context = None):
    ...
    def create_move_from_st_line(self, cr, uid, st_line_id, company_currency_id, st_line_number, context = None):
        ...
        context.update({'res.currency.compute.account': acc_cur})
        amount = res_currency_obj.compute(cr, uid, st.currency.id, company_currency_id, st_line.amount, context=context)
        bank_move_vals = self._prepare_bank_move_line(cr, uid, st_line, move_id, amount, company_currency_id, context=context)
        move_line_id = account_move_line_obj.create(cr, uid, bank_move_vals, context=context)
        torec.append(move_line_id)
        counterpart_move_vals = self._prepare_counterpart_move_line(cr, uid, st_line, move_id, amount, company_currency_id, context=context)
        account_move_line_obj.create(cr, uid, counterpart_move_vals, context=context)
        for line in account_move_line_obj.browse(cr, uid, [ x.id for x in account_move_obj.browse(cr, uid, move_id, context=context).line_id ], context=context):
            if line.state != 'valid':
                raise osv.except_osv(_('Error!'), _('Journal item "%s" is not valid.') % line.name)
    ...
    def balance_check(self, cr, uid, st_id, journal_type = 'bank', context = None):
        ...
        if not (abs((st.balance_end or 0.0) - st.balance_end_real) < 0.0001 or abs((st.balance_end or 0.0) - st.balance_end_real) < 0.0001):
            raise osv.except_osv(_('Error!'), _('The statement balance is incorrect !\nThe expected balance (%.2f) is different than the computed one. (%.2f)') % (st.balance_end_real, st.balance_end))
        return True
    ...
    def button_confirm_bank(self, cr, uid, ids, context = None):
        ...
        for st in self.browse(cr, uid, ids, context=context):
            ...
            self.write(cr, uid, [st.id], {'name': st_number,
             'balance_end_real': st.balance_end}, context=context)
    ...
    def _compute_balance_end_real(self, cr, uid, journal_id, context = None):
        res = False
        if journal_id:
            cr.execute('SELECT balance_end_real                     FROM account_bank_statement                     WHERE journal_id = %s AND NOT state = %s                     ORDER BY date DESC,id DESC LIMIT 1', (journal_id, 'draft'))
            res = cr.fetchone()
...
class account_bank_statement_line(osv.osv):
    ...
    _columns = {'name': fields.char('OBI', required=True, help='Originator to Beneficiary Information'),
     ...
     'move_ids': fields.many2many('account.move', 'account_bank_statement_line_move_rel', 'statement_line_id', 'move_id', 'Moves'),
     'ref': fields.char('Reference', size=32),
     'note': fields.text('Notes'),
     'sequence': fields.integer('Sequence', select=True, help='Gives the sequence order when displaying a list of bank statement lines.'),
     'company_id': fields.related('statement_id', 'company_id', type='many2one', relation='res.company', string='Company', store=True, readonly=True)}
    _defaults = {'name': lambda self, cr, uid, context = {}: self.pool.get('ir.sequence').get(cr, uid, 'account.bank.statement.line'),
